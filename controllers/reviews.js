// import our trail Model so we can find the trail we want to add a review too
...
function deleteReview(req, res) {
  // Finding the trail with the review whose id is sent over in the params of the delete form
  // in the trail  page for delete a review
  ...
  TrailModel.findOne({
    ...
    .then(function (trail) {
      // if we don't find a match, trailDoc would be undefined
      if (!trail) return res.redirect("/trails");

      // remove is method on mongoose subdocuments
      // review is a subdocument
      // its embedded in the trails, One trail has many reviews
      trail.reviews.remove(req.params.id); // <- mutated a document!
      // mongodb doesn't know we removed the review,
      // so we have to call save on the trailDoc to tell mongodb
      trail.save().then(function () {
        res.redirect(`/trails/${trail._id}`); // go back to the trail change where the delete review form was!
      });
    })
    .catch((err) => {
      res.send(err);
...
function create(req, res) {
  ...
  // Use a Model to find the trail with an id (req.params.id)
  TrailModel.findById(req.params.id)
    .then(function (trailDocument) {
      ...
      // doesn't know we added req.body to the trails
      // reviews array
      trailDocument.save().then(function () {
        res.redirect(`/trails/${req.params.id}`);
      });

      // Then with the trail we found, we want to add a review to the trails reviews array
      // to create a review
    })
    .catch((err) => {
      console.log(err);
